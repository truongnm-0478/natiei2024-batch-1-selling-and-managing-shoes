...
function updatePageContent(data) {
    ...
    updateChartData(
        monthlyRevenueChart,
        Object.keys(data.dailyRevenue),
        Object.values(data.dailyRevenue).map((info) => info.currentRevenue)
    );
    ...
    updateChartData(
        ...
        Object.values(data.monthlyRevenue).map((info) => info.currentRevenue)
    );
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    downloadLink.addEventListener("click", function (e) {
        ...
        if (selectedDate) {
            window.location.href = `/admin/statistic/downloadfile?date=${selectedDate}`;
        } else {
            e.preventDefault();
            alert("Vui lòng chọn ngày tháng trước khi xuất file.");
        }
    });
});
