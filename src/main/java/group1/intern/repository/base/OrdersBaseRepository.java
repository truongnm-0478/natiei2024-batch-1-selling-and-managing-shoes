...
public class OrdersBaseRepository implements BaseRepository<Order> {
    ...
    @Override
    public List<Order> fetchAllDataWithoutPagination(List<WhereElements> whereElements, Sort sort, String... relationships) {
        ...
        for (var relationship : relationships) {
            switch (relationship) {
                case "orderDetails":
                    ...
                    for (var order : result)
                        for (var od : order.getOrderDetails())
                            od.setProductQuantity(tmp.stream().filter(pq -> pq.getId().equals(od.getProductQuantity().getId())).findFirst().orElse(null));
            }
        }
        return result;
    }
    ...
    @Override
    public Page<Order> fetchAllDataWithPagination(List<WhereElements> whereElements, Pageable pageable, String... relationships) {
    ...
    @Override
    public List<Order> fetchAllDataWithFirstQuery(List<WhereElements> whereElements, String baseQuery, Sort sort, Pageable pageable) {
        String whereClause = CommonUtils.getWhereClause(whereElements, "o");
        // sort clause
        String sortClause = (sort == null && pageable != null) ? CommonUtils.getSortClause(pageable.getSort(), "o") : CommonUtils.getSortClause(sort, "o");
